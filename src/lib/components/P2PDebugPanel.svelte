...
{#if isDev}
  <div class="fixed bottom-[var(--space-md)] right-[var(--space-md)] z-[60]">
    {#if !open}
      <button
        class="px-[10px] py-[6px] rounded-[var(--radius-full)] border border-[var(--border)] bg-[var(--bg-surface)] text-[var(--text-secondary)] text-[var(--font-size-xs)] transition-all duration-150"
        on:click={() => (open = true)}
    ...
    {:else}
      <div class="w-[320px] rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-overlay)] shadow-[var(--shadow-lg)] overflow-hidden">
        <div class="flex items-center justify-between px-[var(--space-md)] py-[var(--space-sm)] border-b border-[var(--border)]">
          ...
          <button
            class="text-[var(--text-secondary)] px-[8px] py-[4px] rounded-[var(--radius-md)] transition-all duration-150"
            on:click={() => (open = false)}
