...
import { getKnownPeers, saveKnownPeer, getPrivateChat, upsertPrivateChat } from '$lib/services/db.js';
import { buildSessionId, exportPublicKey, generateKeyPair, isSessionActive, resumeSession } from '$lib/services/crypto.js';
import { decryptSealedMessages, setChatOnlineStatus, setKeyExchangeState, upsertChatEntry } from '$lib/stores/privateChatStore.js';
...
function noteConnectFailure(peerId) {
  ...
  recentConnectFailures.set(key, { lastFailedAt: Date.now(), count: (prev?.count ?? 0) + 1 });
...
export function handleIncomingConnection(conn, profile) {
  ...
  if (existing?.connection && existing.connection !== conn) safeClose(existing.connection);
  ...
  conn.on('close', () => {
    ...
    peerStore.update((s) => {
      ...
      if (current?.connection === conn) next.delete(remotePeerId);
...
export async function handleHandshake(msg, fromConn, profile) {
  ...
  if (profile?.username && profile.username !== 'pre-registration' && msg.from.username) {
    ...
    const existingChat = await getPrivateChat(chatId);
    if (existingChat) {
      await upsertPrivateChat({
        ...existingChat,
        myPeerId: get(peerStore).peerId ?? existingChat.myPeerId,
        myUsername: profile.username,
        theirPeerId: remotePeerId,
        theirAvatarBase64: avatarBase64 ?? existingChat.theirAvatarBase64 ?? null
      });
      upsertChatEntry({
        id: chatId,
        theirPeerId: remotePeerId,
...
export async function handleHandshakeAck(msg, fromConn, profile) {
  return await handleHandshake(msg, fromConn, profile);
}

export async function handlePeerDisconnectMessage(msg) {
  ...
  if (wasLobbyHost) setTimeout(electNewLobbyHostFromShared, 2000);
}
