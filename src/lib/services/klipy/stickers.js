 ...
 */
export async function fetchTrendingStickers(limit = 20) {
  const n = Math.max(1, Math.min(50, Math.floor(Number(limit) || 0) || 20));
  const key = cacheKey('stickers_trending', 'trending', n);
...
export async function searchStickers(query, limit = 20) {
  ...
  if (!q) return fetchTrendingStickers(limit);
  const n = Math.max(1, Math.min(50, Math.floor(Number(limit) || 0) || 20));
  const key = cacheKey('stickers_search', q, n);
  const hit = getCached(key);
  if (hit) return /** @type {any} */ (hit);
